...
function preload() {
  backgroundImg = loadImage("./assets/assassins-creed-iv-black-flag-1380569648174_1920x1080-1024x576.png");
shanties = loadSound("./assets/best-sea-shanties.mp3");
...
function setup() {
 shanties.loop(); 
  ...
  player = new Player(285, playerBase.body.position.y - 153, 200, 300);
...
function draw() {
  ...
  textAlign("center");
  textSize(60);
  text("assassins creed IV", width / 2, 100);
...
function keyPressed() {
  if (keyCode === 32) {
    ...
    var arrow = new PlayerArrow(posX, posY, 30, 30, angle);
...
function handleComputerArcher() {
  if (!computerArcher.collapse && !playerArcher.collapse) {
    setTimeout(() => {
      ...
      var arrow = new ComputerArrow(pos.x, pos.y, 30, 30, angle);
